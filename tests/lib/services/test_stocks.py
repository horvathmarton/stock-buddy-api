...
class TestGetPortfolioSnapshot(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.PRICE_SYNCS = data.STOCK_PRICE_SYNCS
        cls.SPLIT_SYNCS = data.STOCK_SPLIT_SYNCS

        cls.snapshot_date = date(2021, 1, 2)
        cls.transactions = [
            StockTransaction(
                amount=2,
                date=date(2021, 1, 2),
    ...
    def test_empty(self):
        self.assertEqual(get_portfolio([], [], self.USERS.owner), {})

    def test_no_actions(self):
        result = get_portfolio([], [self.snapshot_date], self.USERS.owner)

        self.assertEqual(
            result[self.snapshot_date],
            StockPortfolioSnapshot(
                positions={}, date=self.snapshot_date, owner=self.USERS.owner
            ),
        )
    ...
    def test_no_series(self):
        self.assertEqual(
            get_portfolio(
                cast(list[StockTransaction | StockSplit], self.transactions),
                [],
                self.USERS.owner,
            ),
            {},
    ...
    def test_nullify_sold_position(self):
        ...
        self.assertEqual(
            result,
            StockPortfolioSnapshot(
                positions={}, owner=self.USERS.owner, date=snapshot_date
            ),
        )
...
class TestGetPortfolioSnapshotSeries(TestCase):
    ...
    def test_empty_portfolios(self):
        self.assertEqual(
            ...
            StockPortfolioSnapshot(
                positions={}, owner=self.USERS.owner, date=date(2021, 1, 3)
            ),
        )

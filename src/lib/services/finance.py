...
class FinanceService:
    ...
    def get_portfolio_snapshot(self, portfolio: StockPortfolio,
                               at: date = date.today()) -> List[StockPosition]:
        """
        ...
        for transaction in transactions:
            ...
            if ticker not in positions:
                ...
                positions[ticker] = StockPosition(
                    stock=transaction.ticker, shares=transaction.amount, price=latest_price, dividend=latest_dividend * 4, purchase_price=transaction.price, purchase_date=transaction.date)
            else:
                ...
                if transaction.amount >= 0:
                    current_position.purchase_price = (current_position.size_of_position_at_cost + (
                        transaction.amount * transaction.price)) / (current_position.shares + transaction.amount)
    ...
    def _get_latest_stock_price(self, ticker: Stock, at: date) -> float:
        latest_price_date = StockPrice.objects \
            .filter(ticker=ticker, date__lte=at) \
            .latest('date').date
    ...
    def _get_latest_dividend(self, ticker: Stock, at: date) -> float:
        try:
            latest_dividend_date = StockDividend.objects \
                .filter(ticker=ticker, payout_date__lte=at) \
                .latest('payout_date').payout_date

            return StockDividend.objects \
                .filter(ticker=ticker, payout_date=latest_dividend_date)[0].amount
